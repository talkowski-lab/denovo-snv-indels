...
def parent_aware_t_u_annotations_v4(td):
    ...
    td = td.annotate_entries(
        autosomal_copy_state=(
            hl.case()
            .when(
                td.locus.in_autosome() | td.locus.in_x_par() | (td.is_female == True),
                True,
            )
    ...
    td = td.annotate_entries(
        t_from_dad=hl.if_else(
            (
                (td.autosomal_copy_state == True)
                & td.father_entry.GT.is_het()
                & (td.locus.in_x_nonpar() == False)
                & (
                    (td.proband_entry.GT.is_het() & td.mother_entry.GT.is_hom_ref())
                    | (
                        td.proband_entry.GT.is_hom_var()
                        & (
                            td.mother_entry.GT.is_het()
                            | td.mother_entry.GT.is_hom_var()
                        )
                    )
                )
            ),
                ...
                & (
                    ...
                    | (
                        ...
                        & (
                            ...
                            | td.father_entry.GT.is_hom_var()
                        )
                    )
                )
            )
            | (
                (td.autosomal_copy_state == False)
                & td.mother_entry.GT.is_het()
                & td.proband_entry.GT.is_hom_var()
                & (td.father_entry.GT.is_hom_ref() | td.father_entry.GT.is_hom_var())
            ),
            1,
            0,
        ),
...
def filter_vep_to_canonical_transcripts(
    ...
    canonical = mt[vep_root].transcript_consequences.filter(
        lambda csq: csq.CANONICAL == "YES"
    )
    vep_data = mt[vep_root].annotate(
        transcript_consequences=hl.if_else(
            canonical.size() > 0, canonical, mt[vep_root].transcript_consequences
        )
    )
    return (
...
def process_consequences(
    ...
    def find_worst_transcript_consequence(
        ...
        tcl = tcl.map(
            lambda tc: tc.annotate(
                ...
                .when(
                    (tc.BIOTYPE != "protein_coding"), csq_score(tc) + non_coding_score
                )
                ...
                .when(
                    tc.PolyPhen.contains("probably_damaging"), csq_score(tc) - 0.5
                )  # EDITED
    ...
    gene_with_worst_csq = sorted_scores.filter(
        lambda tc: tc.csq_score == lowest_score
    ).map(lambda tc: tc.SYMBOL)
    ensg_with_worst_csq = sorted_scores.filter(
        lambda tc: tc.csq_score == lowest_score
    ).map(lambda tc: tc.Gene)
...
transcript_consequences_strs = transcript_consequences.map(
    lambda x: hl.if_else(
        ...
        hl.struct(
            **{
                col: "." if col != "Consequence" else hl.array(["."])
                for i, col in enumerate(csq_columns)
            }
        ),
    )
...
filt_mt = filt_mt.annotate_rows(
    vep=hl.Struct(transcript_consequences=transcript_consequences_strs)
)
...
base_mt = filt_mt.filter_rows(
    filt_mt.gnomad_non_neuro_AF > gnomad_non_neuro_af_threshold, keep=False
)
...
complete_trio_samples = (
    [trio.s for trio in pedigree.complete_trios()]
...
td = td.annotate_entries(
    total_t_from_parents=td.t_from_dad + td.t_from_mom,
...
inh_td = td.filter_entries(
    (td.total_t_from_parents == 0)
    & (td.total_u_from_parents == 0)
    & (td.t_indeterminate == 0)
    & (td.u_indeterminate == 0),
    keep=False,
)
...
# Cases not in trio
non_trio_cases = ped_ht.filter(
    (ped_ht.phenotype == 2)
    & (~hl.array(complete_trio_samples).contains(ped_ht.sample_id))
).sample_id.collect()
...
all_parent_samples = [
    s
    for s in [trio.pat_id for trio in pedigree.trios]
    + [trio.mat_id for trio in pedigree.trios]
    if s not in ["paternal_id", "maternal_id", None]
]
complete_trio_parent_samples = list(
    np.intersect1d(all_parent_samples, complete_trio_samples)
)
...
if len(control_samples) == 0:
    control_samples = [""]
control_tm = tm.filter_cols(
    (hl.array(control_samples).contains(tm.id))
